...
| FR-21 | Multiple Triggers may fire on one Response; each independent condition is evaluated. Multiple fired Triggers on the same Response surface as a **single Flag** carrying each reason (FR-34, [ADR-0011](../adr/0011-one-flag-per-response.md)), not as separate Flags. | Must |
